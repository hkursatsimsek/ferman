...
| Yakın eşleşmelerde tohum sonucu değiştirmiyor | F0.13'te ölçüldü: 36 eşleşmenin 35'i 1000 tohumun tamamında birebir aynı sonucu üretti (`DeterministicRNG` yalnızca `scatter` için kullanılıyor, çoğu savaşta hiç tüketilmiyor); kendisiyle eşleşmeler bile berabere değil, kesin bir tarafın zaferi (`Balance/phase0-report.md`) | F3.9'un gece denge koşumunda tohumun yanına küçük bir başlangıç konumu jitter'ı ya da koltuk döndürmesi ekle; F0.13'ün kendi sonucunu etkilemiyor (ölçüm zaten oyuncu+düşman koltuğu ortalaması alıyor) |
